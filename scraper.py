...
def scrape_mamikos_single(
    ...
    # ✅ PERBAIKAN: Initialize browser context properly
    ...
    duplicate_exit_threshold = SCRAPER_CONFIG.get("duplicate_exit_threshold", 15)
    ...
    # ✅ PERBAIKAN: Add try-except untuk handle browser errors
    try:
        for region in region_list:
            ...
            # ✅ PERBAIKAN: Check browser context before creating new page
            try:
                page = browser.new_page()
            except Exception as e:
                print(f"    X [Browser Error: {e}]")
                print("    > Recreating browser context...")
                browser.close()
                browser = playwright.chromium.launch_persistent_context(
                    user_data_dir=BROWSER_CONFIG["user_data_dir"],
                    channel=BROWSER_CONFIG["channel"],
            ...
            for idx, card in enumerate(card_iter):
                try:
                    ...
                    if not all(dedup_key):
                        print("    X [Empty Fields: Possible bot detection]")
                    if dedup and dedup_key in seen_keys:
                        ...
                        if duplicate_count > duplicate_exit_threshold:
                            print("    X [Too Many Duplicates: Stopping region]")
                            break
                        continue
                    seen_keys.add(dedup_key)
                    region_results.append(room_data)
                    
                    # Clean success message - show every 10th card or significant milestones
                    if len(region_results) % 10 == 0 or len(region_results) <= 5:
                        print(f"    ✓ [Progress: {len(region_results)} cards completed]")
                    ...
                    print(f"    X [Card Error: {idx} | {str(e)[:30]}...]")
            ...
            # ✅ PERBAIKAN: Ensure page is properly closed
            try:
                page.close()
            except Exception:
        ...
        print(f"  X [FATAL Browser Error: {e}]")
    finally:
        # ✅ PERBAIKAN: Proper cleanup
        try:
            browser.close()
        except Exception:
            pass
...
if __name__ == "__main__":
    ...
    if args.start_end:
        try:
            ...
            print("  X [ERROR Format --start-end: (start:end), Example: --start-end 3:5]")
            sys.exit(1)
